...
def delete_note(request,pk):
    get_object_or_404(Notes, id=pk).delete()
...
def homework(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(request, 'Failed to add homework. Please check the form for errors.')
    else:
        form = HomeworkForm()
...
def update_homework(request, pk):
    homework = get_object_or_404(Homework, id=ObjectId(pk))
